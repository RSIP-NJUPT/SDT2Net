...
from SDT2Net.DTC import DiffRate
from SDT2Net.merge import get_merge_func
from SDT2Net.utils import ste_min

class SDTBlock(Block):
    """
    Modifications:
     - Apply DTC between the attention and mlp blocks
     - Compute and propogate token size and potentially the token sources.
    """
    def introduce_diffrate(self,patch_number, prune_granularity, merge_granularity):
        self.prune_dtc = DiffRate(patch_number,prune_granularity)
        self.merge_dtc = DiffRate(patch_number,merge_granularity)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            ...
            prune_kept_num = self.prune_dtc.update_kept_token_number()      # expected prune compression rate, has gradiet
            ...
            # pruning
            prune_kept_num = self.prune_dtc.kept_token_number
            x = x[:, :prune_kept_num]
            ...
            # merging
            merge_kept_num = self.merge_dtc.kept_token_number
...
def make_diffrate_class(transformer_class):
    class SDTVisionTransformer(transformer_class):
        ...
        def get_kept_num(self):
            ...
            for block in self.blocks:
                prune_kept_num.append(int(block.prune_dtc.kept_token_number))
                merge_kept_num.append(int(block.merge_dtc.kept_token_number))
        ...
        def set_kept_num(self, prune_kept_numbers, merge_kept_numbers):
            ...
            for block, prune_kept_number, merge_kept_number in zip(self.blocks, prune_kept_numbers, merge_kept_numbers):
                block.prune_dtc.kept_token_number = prune_kept_number
                block.merge_dtc.kept_token_number = merge_kept_number


        def calculate_flop_training(self):
            C = self.embed_dim
            patch_number = float(self.patch_embed.num_patches)
            N = torch.tensor(patch_number+1, device=self.blocks[0].prune_dtc.selected_probability.device)
            flops = 0
        ...
        def calculate_flop_inference(self):
            ...
            with torch.cuda.amp.autocast(enabled=False):
                for block in (self.blocks):
                    prune_kept_number = block.prune_dtc.kept_token_number
                    merge_kept_number = block.merge_dtc.kept_token_number
                    mhsa_flops = 4*N*C*C + 2*N*N*C
                    flops += mhsa_flops
                    N = ste_min(N, prune_kept_number, merge_kept_number)
                    ffn_flops = 8*N*C*C
...
def apply_patch(
    ...
    """
    Applies DTC to this transformer.
    """
    SDTVisionTransformer = make_diffrate_class(model.__class__)

    model.__class__ = SDTVisionTransformer
